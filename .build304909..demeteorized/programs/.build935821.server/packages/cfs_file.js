...
FS.File.prototype.copy = function() {                                                                                 // 126
  ...
  var mod, oldKey;                                                                                                    // 145
  for (name in newFile.copies) {                                                                                      // 146
    oldKey = newFile.copies[name].key;                                                                                // 147
    if (oldKey) {                                                                                                     // 148
      // We need to ask the adapter for the true oldKey because right now gridfs does some extra stuff.               // 149
      // TODO GridFS should probably set the full key object (with _id and filename) into `copies.key`                // 150
      // so that copies.key can be passed directly to createReadStreamForFileKey                                      // 151
      var sourceFileStorage = self.collection.storesLookup[name];                                                     // 152
      if (!sourceFileStorage) {                                                                                       // 153
        throw new Error(name + " is not a valid store name");                                                         // 154
      }                                                                                                               // 155
      oldKey = sourceFileStorage.adapter.fileKey(self);                                                               // 156
      delete newFile.copies[name].key; // delete so that new fileKey will be generated in copyStoreData               // 157
      mod = mod || {};                                                                                                // 158
      mod["copies." + name + ".key"] = copyStoreData(newFile, name, oldKey);                                          // 159
    }                                                                                                                 // 160
  }                                                                                                                   // 161
  // Update keys in the filerecord                                                                                    // 162
  if (mod) {                                                                                                          // 163
    newFile.update({$set: mod});                                                                                      // 164
  }                                                                                                                   // 165
                                                                                                                      // 166
  return newFile;                                                                                                     // 167
};                                                                                                                    // 168
                                                                                                                      // 169
Meteor.methods({                                                                                                      // 170
  // Does a HEAD request to URL to get the type, updatedAt, and size prior to actually downloading the data.          // 171
  // That way we can do filter checks without actually downloading.                                                   // 172
  '_cfs_getUrlInfo': function (url, options) {                                                                        // 173
    this.unblock();                                                                                                   // 174
                                                                                                                      // 175
    var response = HTTP.call("HEAD", url, options);                                                                   // 176
    var headers = response.headers;                                                                                   // 177
    var result = {};                                                                                                  // 178
                                                                                                                      // 179
    if (headers['content-type']) {                                                                                    // 180
      result.type = headers['content-type'];                                                                          // 181
    }                                                                                                                 // 182
                                                                                                                      // 183
    if (headers['content-length']) {                                                                                  // 184
      result.size = +headers['content-length'];                                                                       // 185
    }                                                                                                                 // 186
                                                                                                                      // 187
    if (headers['last-modified']) {                                                                                   // 188
      result.updatedAt = new Date(headers['last-modified']);                                                          // 189
    }                                                                                                                 // 190
                                                                                                                      // 191
    return result;                                                                                                    // 192
  }                                                                                                                   // 193
                                                                                                                      ...
                                                                                                                      // 195
// TODO maybe this should be in cfs-storage-adapter                                                                   // 196
function _copyStoreData(fileObj, storeName, sourceKey, callback) {                                                    // 197
  if (!fileObj.isMounted()) {                                                                                         // 198
    throw new Error("Cannot copy store data for a file that is not associated with a collection");                    // 199
  }                                                                                                                   // 200
                                                                                                                      // 201
  var storage = fileObj.collection.storesLookup[storeName];                                                           // 202
  if (!storage) {                                                                                                     // 203
    throw new Error(storeName + " is not a valid store name");                                                        // 204
  }                                                                                                                   // 205
                                                                                                                      // 206
  // We want to prevent beforeWrite and transformWrite from running, so                                               // 207
  // we interact directly with the store.                                                                             // 208
  var destinationKey = storage.adapter.fileKey(fileObj);                                                              // 209
  var readStream = storage.adapter.createReadStreamForFileKey(sourceKey);                                             // 210
  var writeStream = storage.adapter.createWriteStreamForFileKey(destinationKey);                                      // 211
                                                                                                                      // 212
  writeStream.once('stored', function(result) {                                                                       // 213
    callback(null, result.fileKey);                                                                                   // 214
  });                                                                                                                 // 215
                                                                                                                      // 216
  writeStream.once('error', function(error) {                                                                         // 217
    callback(error);                                                                                                  // 218
  });                                                                                                                 // 219
                                                                                                                      // 220
  readStream.pipe(writeStream);                                                                                       // 221
}                                                                                                                     // 222
var copyStoreData = Meteor._wrapAsync(_copyStoreData);                                                                // 223
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
