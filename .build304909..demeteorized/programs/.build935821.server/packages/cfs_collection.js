...
FS.Collection = function(name, options) {                                                                              // 12
  ...
  self.files = new Meteor.Collection(collectionName, _filesOptions);                                                   // 112
                                                                                                                       // 113
  ...
  Meteor.isServer && FS.FileWorker && FS.FileWorker.observe(this);                                                     // 129
                                                                                                                       // 130
  // Emit events based on TempStore events                                                                             // 131
  if (FS.TempStore) {                                                                                                  // 132
    FS.TempStore.on('stored', function (fileObj, result) {                                                             // 133
      ...
      if (fileObj.collectionName === name) {                                                                           // 135
        var emitted = self.emit('uploaded', fileObj);                                                                  // 136
        if (FS.debug && !emitted) {                                                                                    // 137
          console.log(fileObj.name() + ' was successfully uploaded. You are seeing this informational message because you enabled debugging and you have not defined any listeners for the "uploaded" event on the ' + name + ' collection.');
                                                                                                                       ...
                                                                                                                       // 142
    FS.TempStore.on('error', function (error, fileObj) {                                                               // 143
      // When a file has an error while being stored into the temp store, we emit an "error" event on the FS.Collection only if the file belongs to this collection
      if (fileObj.collectionName === name) {                                                                           // 145
        self.emit('error', new Error('Error storing uploaded file to TempStore: ' + error.message), fileObj);          // 146
      }                                                                                                                // 147
    });                                                                                                                // 148
  } else if (Meteor.isServer) {                                                                                        // 149
    throw new Error("FS.Collection constructor: FS.TempStore must be defined before constructing any FS.Collections.") // 150
  }                                                                                                                    // 151
                                                                                                                       // 152
};                                                                                                                     // 153
                                                                                                                       // 154
// An FS.Collection can emit events                                                                                    // 155
FS.Collection.prototype = new EventEmitter();                                                                          // 156
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
