...
FS.TempStore.createWriteStream = function(fileObj, options) {                                                          // 244
  ...
  writeStream.on('error', function (error) {                                                                           // 333
    self.emit('error', error, fileObj);                                                                                // 334
  });                                                                                                                  // 335
                                                                                                                       // 336
  return writeStream;                                                                                                  // 337
};                                                                                                                     // 338
                                                                                                                       // 339
/**                                                                                                                    // 340
  * @method FS.TempStore.createReadStream                                                                              // 341
  * @public                                                                                                            // 342
  * @param {FS.File} fileObj The file to read                                                                          // 343
  * @return {Stream} Returns readable stream                                                                           // 344
  *                                                                                                                    // 345
  */                                                                                                                   // 346
FS.TempStore.createReadStream = function(fileObj) {                                                                    // 347
  // Ensure that we have a storage adapter mounted; if not, throw an error.                                            // 348
  mountStorage();                                                                                                      // 349
                                                                                                                       // 350
  // If fileObj is not mounted or can't be, throw an error                                                             // 351
  mountFile(fileObj, 'FS.TempStore.createReadStream');                                                                 // 352
                                                                                                                       // 353
  FS.debug && console.log('FS.TempStore creating read stream for ' + fileObj._id);                                     // 354
                                                                                                                       // 355
  // Determine how many total chunks there are from the tracker collection                                             // 356
  var chunkInfo = tracker.findOne({fileId: fileObj._id, collectionName: fileObj.collectionName}) || {};                // 357
  var totalChunks = FS.Utility.size(chunkInfo.keys);                                                                   // 358
                                                                                                                       // 359
  function getNextStreamFunc(chunk) {                                                                                  // 360
    return Meteor.bindEnvironment(function(next) {                                                                     // 361
      var fileKey = _fileReference(fileObj, chunk);                                                                    // 362
      var chunkReadStream = FS.TempStore.Storage.adapter.createReadStream(fileKey);                                    // 363
      next(chunkReadStream);                                                                                           // 364
    }, function (error) {                                                                                              // 365
      throw error;                                                                                                     // 366
    });                                                                                                                // 367
  }                                                                                                                    // 368
                                                                                                                       // 369
  // Make a combined stream                                                                                            // 370
  var combinedStream = CombinedStream.create();                                                                        // 371
                                                                                                                       // 372
  // Add each chunk stream to the combined stream when the previous chunk stream ends                                  // 373
  var currentChunk = 0;                                                                                                // 374
  for (var chunk = 0; chunk < totalChunks; chunk++) {                                                                  // 375
    combinedStream.append(getNextStreamFunc(chunk));                                                                   // 376
  }                                                                                                                    // 377
                                                                                                                       // 378
  // Return the combined stream                                                                                        // 379
  return combinedStream;                                                                                               // 380
};                                                                                                                     // 381
                                                                                                                       // 382
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
